...
function renderLabel(options: MarkdownItLabelOptions) {
  return function renderLabelToken(
    tokens: Token[],
    idx: number,
    ...
    const labelColor = token.meta.color ??
      parseLabelColor(env.labels[token.meta.text]) ?? [128, 128, 128];

    let textColorClass = getContrastColor(
      labelColor,
      options.cssClassTextLight,
      options.cssClassTextDark,
    );
...
function parseLabel(state: StateInline, silent: boolean) {
  ...
  let labelText = state.md.utils.unescapeAll(
 ...
 */
export default function labelPlugin(
  md: MarkdownIt,
  options: MarkdownItLabelOptions,
) {
